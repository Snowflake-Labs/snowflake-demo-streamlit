...
role must hold `USAGE` and `EMBED` on the app. "App not found" and "insufficient
privileges" both return **`403 FORBIDDEN`** (deliberately identical, so app existence
isn't leaked).
...
-- allows viewing it in Snowflake
...
- `X-Snowflake-Role` is **required**, and that role needs `USAGE` and `EMBED` on the
  app. Both a missing app and missing privileges return **`403`** (identical by design).
...
- The routes return a generic `500` and log the real error server-side. Don't echo
  ...
  app, and role, which defeats the deliberately indistinguishable `403`, and key-read
  failures disclose the private key's path. Read the server log when debugging.
